  ...
  <div class="p-6 max-w-full">
    ...
    <div class="flex border-b border-gray-200 mb-6 overflow-x-auto">
      <button @click="activeTab = 'nuevo'"
        :class="['whitespace-nowrap px-6 py-3 font-bold text-sm transition-colors', activeTab === 'nuevo' ? 'text-[#FF6B2B] border-b-2 border-[#FF6B2B]' : 'text-gray-500 hover:text-gray-700']">
        📝 Nuevo Pedido
      </button>
      <button @click="activeTab = 'pendientes'; cargarPendientes()"
        :class="['whitespace-nowrap px-6 py-3 font-bold text-sm transition-colors relative', activeTab === 'pendientes' ? 'text-[#FF6B2B] border-b-2 border-[#FF6B2B]' : 'text-gray-500 hover:text-gray-700']">
        📦 Pendientes
      ...
      <button @click="activeTab = 'historial'; cargarHistorial()"
        :class="['whitespace-nowrap px-6 py-3 font-bold text-sm transition-colors', activeTab === 'historial' ? 'text-[#FF6B2B] border-b-2 border-[#FF6B2B]' : 'text-gray-500 hover:text-gray-700']">
        📊 Mi Historial
      </button>
    </div>
    ...
    <div class="bg-white rounded-2xl p-6 border border-[#FFE0CC] shadow-sm">
      ...
      <div v-if="activeTab === 'nuevo'">
        ...
        <div v-if="ultimoPedidoGuardado"
          class="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl flex items-center justify-between">
          <div>
          ...
          <button @click="pedidosService.descargarPDF(ultimoPedidoGuardado.id)"
            class="bg-white border border-green-300 text-green-700 font-bold text-sm px-4 py-2 rounded-xl hover:bg-green-100 transition-colors">
            📄 PDF
          </button>
        </div>
      </div>
          ...
          <div v-for="pedido in historialFiltrado" :key="pedido.id"
            ...
            <div class="flex items-center justify-between p-4 cursor-pointer"
              ...
              <div class="text-right">
                <!-- Si fue entregado, mostrar total de venta -->
                <div v-if="pedido.estado === 'entregado' && pedido.ventas" class="font-black text-green-600">
            ...
            <div v-if="historialAbierto === pedido.id" class="px-4 pb-4 border-t border-gray-100 pt-4">

              <!-- Info de venta si fue entregado -->
              <div v-if="pedido.estado === 'entregado' && pedido.ventas"
                ...
                <div class="text-right">
                  <div class="text-xl font-black text-green-700">Bs. {{ pedido.ventas.total.toFixed(2) }}</div>
                </div>
              </div>
              ...
              <!-- Botón PDF si fue entregado -->
              <div v-if="pedido.estado === 'entregado'" class="mt-4">
                <button @click="pedidosService.descargarPDF(pedido.id)"
                  class="w-full bg-white border border-[#FFE0CC] text-[#FF6B2B] font-bold py-2 rounded-xl hover:bg-orange-50 transition-colors text-sm">
...
const guardarPedido = async () => {
  ...
  try {
    ...
    limpiarPedido()
    alert(`✅ Pedido ${resultado.numero} guardado`)
  } catch (e) {
